...
type
  ...
  IOrderSQL = interface
    ...
    function Asc(): IOrderSQL;
    function Desc(): IOrderSQL;
    function _Get(const AColumnName: String): IOrderSQL;
  end;

  OrderSQL = class(TInterfacedObject,IOrderSQL)
  private
    FOrder: String;
    constructor Create(const AOrder: String);
  public
    function Parse: String;
    function Asc(): IOrderSQL;
    function Desc(): IOrderSQL;
    function _Get(const AColumnName: String): IOrderSQL;
  public
    class function Get(const AColumnName: String): IOrderSQL;
  end;
...
function OrderSQL.Asc: IOrderSQL;
begin
  Result := Self;
  FOrder := FOrder + ' ASC';
end;

constructor OrderSQL.Create(const AOrder: String);
begin
  FOrder := AOrder;
end;

function OrderSQL.Desc: IOrderSQL;
begin
  Result := Self;
  FOrder := FOrder + ' DESC';
end;

class function OrderSQL.Get(const AColumnName: String): IOrderSQL;
begin
  Result := Self.Create(AColumnName);
end;

function OrderSQL.Parse: String;
begin
  Result := FOrder;
end;

function OrderSQL._Get(const AColumnName: String): IOrderSQL;
begin
  Result := Self;
  FOrder := FOrder + ',' + AColumnName;
end;
